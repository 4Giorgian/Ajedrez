...
public class ChessGameBoard extends JPanel{
    ...
    public void resetBoard ( boolean addAfterReset ){
        AbstractElementsBoardFactory SquaresFactory = ElementsBoardProductor.getTypeElementBoard("SQUARE");
        chessCells = new BoardSquare[8][8];
        this.removeAll();
        ...
        for ( int i = 0; i < chessCells.length; i++ ){
            for ( int j = 0; j < chessCells[0].length; j++ ){
                chessCells[i][j] = SquaresFactory.createBoardSquare(i, j, null);
                if ( ( i + j ) % 2 == 0 ){
    ...
    public void initializeBoard(){
        AbstractElementsBoardFactory PiecesFactory = ElementsBoardProductor.getTypeElementBoard("PIECE");
        AbstractElementsBoardFactory SquaresFactory = ElementsBoardProductor.getTypeElementBoard("SQUARE");
        resetBoard( false );
        for ( int i = 0; i < chessCells.length; i++ ){
            for ( int j = 0; j < chessCells[0].length; j++ ){
                ...
                else if ( i == 0 || i == 7 ) // main rows
                {
                    ...
                    if ( j == 0 || j == 7 ){
                        pieceToAdd = PiecesFactory.CreateStandarPiece("ROOK", this, i, j, colNum);
                    }
                    else if ( j == 1 || j == 6 ){
                        pieceToAdd = PiecesFactory.CreateStandarPiece("KNIGHT", this, i, j, colNum);
                    }
                    else if ( j == 2 || j == 5 ){
                        pieceToAdd = PiecesFactory.CreateStandarPiece("BISHOP", this, i, j, colNum);
                    }
                    else if ( j == 3 ){
                        pieceToAdd = PiecesFactory.CreateStandarPiece("KING", this, i, j, colNum);
                    }
                    else
                    {
                        pieceToAdd = PiecesFactory.CreateStandarPiece("QUEEN", this, i, j, colNum);
                    }
                }
                else
                {
                ...
                chessCells[i][j] = SquaresFactory.createBoardSquare(i, j, pieceToAdd);
                if ( ( i + j ) % 2 == 0 ){
                    chessCells[i][j].setBackground( Color.WHITE );
                }
                else
